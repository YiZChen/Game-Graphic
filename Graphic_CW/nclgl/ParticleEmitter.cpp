...
ParticleEmitter::ParticleEmitter(bool isareaSpawn, char* texturePath, bool isAllDirectionSpawn) {
	particleRate = 100.0f;
	...
	if (isAllDirectionSpawn)
	{
		isAllDirectionSpawner = true;
	}
	else
	{
		isAllDirectionSpawner = false;
	}
	...
	*/
	...
	ParticleTexture = SOIL_load_OGL_texture(texturePath,
		SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID, SOIL_FLAG_COMPRESS_TO_DXT);
...
void ParticleEmitter::Update(float msec) {
	...
	*/
	...
	for (std::vector<Particle *>::iterator i = particles.begin(); i != particles.end();/*No I++ here!!! */) {
		...
		if (p->colour.w <= 0.0f || (p->position.y < 0.0f)) {
			...
			if (particleSpeed != 0)
			{
				...
				if (isAreaSpawner)
				{
					p->position.x += msec * 0.05f * windEffect * (rand() % 5);
				}
				else
				{
					p->position.x += msec * 0.01f * windEffect * RAND();
				}

				p->position.y -= (msec * msec * msec * p->gravity) / 10000;

				p->gravity += msec * 0.05f;
			}
...
Particle* ParticleEmitter::GetFreeParticle() {
	...
	{
		if (isAllDirectionSpawner)
		{
			p->colour = Vector4((RAND() - RAND()) + 0.5, 0.05, 0.05, 0.5);
		...
		{
			p->colour = Vector4((RAND() - RAND()) + 0.5, 0.05, 0.05, 0.5);
			p->direction.x += ((RAND() - RAND()) / 1.5 * particleVariance);
			p->direction.y += (1 * particleVariance);
			p->direction.z += ((RAND() - RAND()) / 1.5 * particleVariance);
		}
	}
	...
	{
		//random pick spawn point from a 4096 * 4096 area
		p->position = Vector3((rand() % 4500), 2000 - rand() % 50, (rand() % 4500));
	}
...
void ParticleEmitter::ResizeArrays() {
	...
	glDeleteBuffers(1, &VBOId[VERTEX_BUFFER]);
	glDeleteBuffers(1, &VBOId[COLOUR_BUFFER]);
